...
namespace Swashbuckle.MicrosoftExtensions.Tests
{
    public class MetadataAttributeTests
    {
        ...
        public async void PropertyMetadata()
        {
            ...
            Assert.Equal("customName", testyProperty.Key);
            Assert.Equal("advanced", testyProperty.Value.Extensions["x-ms-visibility"].ToString());
            Assert.Equal("Friendly", testyProperty.Value.Extensions["x-ms-summary"].ToString());
